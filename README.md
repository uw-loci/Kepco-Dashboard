...
Before preview or upload, the app validates the requested waveform symmetrically against the configured absolute limit for the selected control mode. On disconnect or application close from a verified session, it stops LIST mode, zeros only the active fixed-output channel, sends `OUTP OFF`, and verifies `OUTP?`, `MEAS:VOLT?`, and `MEAS:CURR?`. This preserves the complementary hardware limit and avoids programming the inactive channel during shutdown. If verification fails, disconnect or close is blocked so the operator can resolve the unsafe state. If the session is already unverified, no output commands can be issued safely; disconnect or close instead retires the untrusted socket with the physical output state marked unknown.
...
Dependent DC state changes do not rely on the `35 ms` pacing delay. If a mode transition is actually needed, the controller sends `FUNC:MODE`, waits with `*WAI`, and verifies `FUNC:MODE?`; it changes the selected source to `MODE FIX` only when necessary, waits again, and verifies the source `MODE?`. Initial DC/LIST staging then follows the manual's optimized sequence: lock the active source to full scale with `RANG 1`, stage the operating parameter at zero, and program the one complementary limit to its desired absolute maximum. For DC output enable, the requested operating setpoint is programmed and verified while output remains off, then `OUTP ON` is sent. Later same-mode DC updates change only the operating parameter; the complementary limit is rewritten only when the operator changes it. Live DC mode and range changes are rejected until output is disabled. LIST shutdown disarms the selected LIST source, sends `OUTP OFF`, waits with `*WAI`, and requires `OUTP?` to report OFF before the dashboard reports a successful stop. Polling is paused for DC staging, mode/range/limit changes, output transitions, reset/manual writes and queries, recovery, and disconnect. It resumes after a verified postflight snapshot or, when the socket remains healthy, after logging a command rejection. BIT `-221,"Settings conflict"` is an execution error and blocks the programming action that produced it, just like other device command errors. Cleanly received SCPI rejections leave the communication session healthy but prevent the requested upload or state change from being committed. Transport framing failures, timeouts, invalid responses, and socket failures enter `DEGRADED` and require fresh-socket recovery.
...
All device-facing Manual Override actions run on background workers. An application-level operation gate prevents manual actions from being queued during upload, output transitions, scanning, connect/disconnect, or recovery, and prevents those primary workflows from starting while a manual action is awaiting completion. Successful arbitrary SCPI writes clear waveform readiness because the dashboard cannot infer whether the command changed LIST data, dwell, mode, range, output, or a staged DC setpoint. A new verified upload is required before normal output enable is available again.
...
- Device-facing Manual Override callbacks must reserve `DashboardApp._manual_operation_in_flight`, run through `_start_manual_operation`, and release the gate only in the queued UI completion callback.
...
- A new `-221,"Settings conflict"` or other device execution error aborts and visibly logs the requested action without forcing communication recovery. Pre-existing queue entries are drained and logged before programming so they are not attributed to the new action.
- Verify the requested hardware state before trying a corrective command after any device execution error.
