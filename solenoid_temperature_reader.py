"""Read solenoid temperatures from EBEAM dashboard datalog JSONL files."""
...
class DatalogSolenoidTemperatureReader:
    """Read the most recent solenoid temperature values from EBEAM datalogs."""

    def __init__(self, datalog_dir: Optional[Union[str, Path]] = None):
        self.datalog_dir = (
            Path(datalog_dir) if datalog_dir is not None else self.default_datalog_dir()
        )

    @staticmethod
    def default_datalog_dir() -> Path:
        """Match the EBEAM dashboard's datalog directory."""
        return Path.home() / EBEAM_DASHBOARD_DIR / DATALOG_DIR

    def read_latest(self) -> SolenoidTemperatureSnapshot:
        """Return solenoid 1/2 values from the newest valid datalog entry."""
        try:
            datalog_files = self._find_datalog_files_newest_first()
            if not datalog_files:
                return SolenoidTemperatureSnapshot(
                    error=f"No {DATALOG_FILE_PATTERN} files found in {self.datalog_dir}"
                )
            ...
            return SolenoidTemperatureSnapshot(
                error=f"Unable to read datalog directory {self.datalog_dir}: {exc}"
            )
